...
void render_calendar(SDL_Renderer *renderer, TTF_Font *font, bool *running, int window_width, int window_height, AppState *currentState) {
    ...
    SDL_Event event;
    bool in_calendar = true;
    
    // Boucle principale de gestion des événements
    while (in_calendar && *running) {
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_QUIT) {
                *running = false;
                ...
                // Vérifier si la barre de recherche est cliquée pour revenir à l'accueil
                if (x >= 0 && x <= window_width && y >= 0 && y <= 50) {
                    printf("Retour à l'accueil\n");
                    *currentState = STATE_HOME;  // Passer à l'état d'accueil
                    in_calendar = false; 
                }
            }
        }
