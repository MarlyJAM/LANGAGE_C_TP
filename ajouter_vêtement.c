        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_MOUSEBUTTONDOWN || event.type == SDL_FINGERDOWN) {
                ...
                if (x >= buttonRect.x && x <= buttonRect.x + buttonRect.w &&
                    ...
                    //     const char *fileName = strrchr(file_path, '/');  // Pour Linux/Mac
                    //     if (!fileName) {
                    //         fileName = strrchr(file_path, '\\');  // Pour Windows
                    //     }

                    //     // Si un nom de fichier a été trouvé, on passe au caractère suivant après '/' ou '\\'
                    //     if (fileName) {
                    //         fileName++;  // Sauter le '/' ou '\\'
                    //     } else {
                    //         fileName = file_path;  // Aucun séparateur trouvé, donc le chemin complet est le nom du fichier
                    //     }

                    //     // Copier le nom du fichier dans la structure
                    //     strncpy(newVetement.image, fileName, sizeof(newVetement.image) - 1);
                    //     newVetement.image[sizeof(newVetement.image) - 1] = '\0';  // Assurer la fin de chaîne
                    // } else {
